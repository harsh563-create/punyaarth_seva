/* Deterministic pseudo-random so SSR/hydration and re-renders are stable.
   Uses integer-only math: Math.sin and friends may differ by an ULP between
   the Node runtime and the browser, which breaks attribute-level hydration
   matching. Basic integer ops are bit-exact everywhere. */
function rand(seed: number): number {
  let h = Math.imul(Math.round(seed * 4096) | 0, 0x85ebca6b);
      ...
      <g>
        ...
        <g stroke="#4f9a52" strokeLinecap="round" fill="none">
          {Array.from({ length: 34 }, (_, i) => {
            ...
            const d = `M ${gx.toFixed(2)} ${gy.toFixed(2)} q ${(lean * 0.4).toFixed(2)} ${(-h * 0.6).toFixed(2)} ${lean.toFixed(2)} ${(-h).toFixed(2)}`;
            return (
              <path
                key={`gr${i}`}
                d={d}
                strokeWidth={2.2}
                opacity={Number((0.55 + rand(i * 1.9) * 0.35).toFixed(2))}
                className="grass-blade"
                style={{
                  animationDelay: `${(i % 9) * 0.45}s`,
                  animationDuration: `${3.4 + (i % 5) * 0.5}s`,
                  transformOrigin: `${gx.toFixed(2)}px ${gy.toFixed(2)}px`,
                }}
              />
            );
          })}
